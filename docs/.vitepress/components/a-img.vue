...
interface Props {
  ...
  dir?: string
  hide?: boolean
  fl?: number // 焦距 focal length
  cf?: number // 系数 crop factor
  f?: number // 光圈 aperture (f-number)
}
...
const equivalentFocalLength = computed(() => {
  if (!props.fl) return null
  const cropFactor = props.cf || 1 // 默认系数为1
  return Math.round(props.fl * cropFactor)
})
    ...
    <a-modal v-if="!loading || (src || srcMin)" :scale="scale" @popover="handlePopoverOpen">
      <template #action>
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <!-- 照片元数据显示区域 -->
          <div class="a-img-metadata" v-if="equivalentFocalLength || props.f">
            <span v-if="equivalentFocalLength">{{ equivalentFocalLength }}mm</span>
            <span v-if="equivalentFocalLength && props.f" style="margin: 0 4px;"></span>
            <span v-if="props.f">ƒ/{{ props.f }}</span>
          </div>
